...
<!--Segue into frequency table-->

```{r frequencyTableDecade, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
# Install and load required packages
packages_to_install <- c("tidyverse", "dplyr", "ggplot2", "lubridate", "knitr")
...
kable(summary_stats_decades, "latex", booktabs = TRUE, escape = FALSE)

```
The Decade Frequency table shown above shows the statistics of the popularity of songs in each decade. This information is important for tracking shifts in musical preferences over the decades and understanding what appealed to audiences in different eras. This can show evolving genres which could help to understand how a song could be made more popular. Another useful takeaway from this table is that comparing the statistics across decades allows for a comparative analysis. It can help pinpoint changes in the distribution of popular songs-whether there's a widening or narrowing gap in popularity among songs within each decade. One thing we noticed was that there was a noticeable increase in the number of songs per decade indicating a growth in music production.  
  
The Genre Frequency table that is shown below is the same table but instead of looking at decades, it looks at the genres across all of the decades. With this information, we can explore which genres consistently get more attention over time and which ones rise and fall in popularity over time. Also comparing the statistics like quartiles and median popularity among genres it enables us to identify any disparities or similarities in popularity distributions. This will help us in recognizing which genres have broader appeals. One thing we noticed was that EDM shows the widest range in popularity scores. Also, Latin and Pop genres have higher median and upper quartile popularity scores compared to R&B, Rap, and Rock. This suggests a consistently higher popularity for Latin and Pop genres.  
  
```{r frequencyTableGenre, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
# Install and load required packages
packages_to_install <- c("tidyverse", "dplyr", "ggplot2", "lubridate", "knitr")
...
# Print nicely formatted table for Genres
kable(summary_stats_genres, "latex", booktabs = TRUE, escape = FALSE)

```
  
Combining both of these frequency tables allows us to make insightful conclusions on what makes a song popular. While looking at both tables, there seems to be a convergence in popularity scores within decades. When considering genres, there is a distinct patterns in popularity which suggest that genre-specific factors significantly influence a song’s popularity.
...
```{r genreTrendChart, echo=FALSE, warning=FALSE, message=FALSE}
options(repos = c(CRAN = "https://cran.rstudio.com"))
# Load data from GitHub
github_url <- "https://raw.githubusercontent.com/Stat184-Fall2023/FP-LiamHooks/main/Spotify%20Songs%20NEW.csv"
spotify_data <- read.csv(github_url)
